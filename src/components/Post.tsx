...
export function Post({ post }: PostProps) {
  const [ comments, setComments ] = useState<string[]>(["oi"]) ;
  const [ newCommentText, setNewCommentText ] = useState<string>("");

  const publisehdDateFormated = format(post.publishedAt, "d 'de' LLLL 'às' HH:mm'h'", {
    locale: ptBR
  })
  ...
  function deleteComment(commentToDelete: string){
    const commentsWithoutDeletedOne = comments.filter((comment: string) => {
